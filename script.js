...
let FloatClass = ['float-none', 'float-start', 'float-end', 'mt-5', 'float-end', 'float-start', 'float-none', 'mt-4'];
...
function keyboard(e) {
    if ((e.key).toUpperCase() == text.innerHTML) {
        ...
        if (answerFalse == 20) {
            ...
            alert(user + ' sizda juda ham ko\'p xatolik bo\'ldi! Qaytadan harakat qiling');
            location.reload();
        }
    }
    if (answerTrue == 1) {
...
function stopkeyboard() {
    ...
    sendtelegram(`${user} (${Localuser}) ${setTime} sekund vaqt davomida, ${answerTrue} ta to'g'ri va ${answerFalse} ta xato bajardi `);
...
// sendtelegram
function sendtelegram(message) { let telegram_bot_id = "6243270315:AAHIWu64G4jafCLI-St_drgPicxJ70pbLDo"; let chat_id = 6034632659; let settings = { "async": true, "crossDomain": true, "url": "https://api.telegram.org/bot" + telegram_bot_id + "/sendMessage", "method": "POST", "headers": { "Content-Type": "application/json", "cache-control": "no-cache" }, "data": JSON.stringify({ "chat_id": chat_id, "text": message }) }; $.ajax(settings).done(function (response) { }); };
